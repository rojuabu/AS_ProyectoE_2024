...
aplic_dwt <- function(imag, wf, nivel) {
...
comprimir_imag <- function(dwt_imagen, filtroQ = FALSE, q = -1) {
  if (!(filtroQ)) {
...
reconstruir_imag <- function(dwt_imagen, dim) {
...
La función `aplic_dwt` aplica la Transformada Wavelet Discreta en 2D a los tres canales de la imagen, especificado el filtro wavelet (wf) y el nivel.

La función `comprimir_imag` comprime la imagen, especificando el filtro a usar. Si filtroQ es FALSE, se usa el filtro `filtro2d_paso_bajo`, y si filtroQ es TRUE, se usa el filtro `filtro2d_paso_bajo2`. En este último caso, también se deberá especificar el cuantil que se quiera usar en el filtro.

La función `reconstruir_imag` reconstruye la imagen por canal dada la dimensión de la imagen y dadas las transformadas Wavelet discretas en 2D de sus tres canales.
...
dwt_imagen <- aplic_dwt(matriz_imagen, "haar", 1)
imagen_comp1 <- comprimir_imag(dwt_imagen)
imagen_rec1 <- reconstruir_imag(imagen_comp1, dim)
q <- c(0.5, 0.7, 0.9)
imagen_comp21 <- comprimir_imag(dwt_imagen, TRUE, q[1])
imagen_rec21 <- reconstruir_imag(imagen_comp21, dim)
imagen_comp22 <- comprimir_imag(dwt_imagen, TRUE, q[2])
imagen_rec22 <- reconstruir_imag(imagen_comp22, dim)
imagen_comp23 <- comprimir_imag(dwt_imagen, TRUE, q[3])
imagen_rec23 <- reconstruir_imag(imagen_comp23, dim)
```
...
mse_imags <- function(imag1, imag2) {
  return(mean(c(mean((imag1[,,1] - imag2[,,1])^2), mean((imag1[,,2] - imag2[,,2])^2), mean((imag1[,,3] - imag2[,,3])^2))))
}

mae_imags <- function(imag1, imag2) {
  return(mean(c(mean(abs(imag1[,,1] - imag2[,,1])), mean(abs(imag1[,,2] - imag2[,,2])), mean(abs(imag1[,,3] - imag2[,,3])))))
}

psnr_imags <- function(imag1, imag2) {
  mse <- mse_imags(imag1, imag2)
  max_intensidad <- 255
  return(10 * log10((max_intensidad^2) / mse))
}

calc_errores <- function(imag1, imag2) {
  return(c("MSE" = mse_imags(imag1, imag2), "MAE" = mae_imags(imag1, imag2), "PSNR" = psnr_imags(imag1, imag2)))
}
```
...
```{r 3_2_1_errores_en_ejemplo_3_1, echo=FALSE}
print("Error filtro paso bajo:")
calc_errores(matriz_imagen, imagen_rec1)
print("Error filtro paso bajo 2:")
calc_errores(matriz_imagen, imagen_rec2)
...
for (i in 1:length(imags)) {
  ...
  dwt_imagen <- aplic_dwt(imag, "haar", 1)
  imagen_comp1 <- comprimir_imag(dwt_imagen)
  imagen_rec1 <- reconstruir_imag(imagen_comp1, dim)
  
  imagen_comp2 <- comprimir_imag(dwt_imagen, TRUE, q)
  imagen_rec2 <- reconstruir_imag(imagen_comp2, dim)
