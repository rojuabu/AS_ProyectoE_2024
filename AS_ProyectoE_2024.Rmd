...
Procedemos a comprimir y a reconstruir la imagen usada en el apartado anterior, utilizando los dos filtros que actúan sobre la transformada wavelet discreta en 2D de la imagen. El segundo filtro, se probará para varios cuantiles (0.5, 0.7 y 0.9).
...
Asimismo, procedemos a mostrar la imagen comprimida con el filtro de paso bajo reconstruida, y las imágenes comprimidas con el filtro de paso bajo utilizando el umbral reconstruidas.

Reconstrucción usando el primer filtro:
...
Reconstrucción usando el segundo filtro con `q = 0.5`:
...
Reconstrucción usando el segundo filtro con `q = 0.7`:
...
Reconstrucción usando el segundo filtro con `q = 0.9`:
...
Procedemos a calcular los errores MSE y MAE, y el PSNR entre la imagen original y las imágenes reconstruidas.
...
Ahora, se procede a generar una tabla que contiene información sobre el MSE, MAE y PSNR entre cada imagen en color y la imagen reconstruida obtenida a partir de esta con el método 1 (usando el filtro `filtro2d_paso_bajo`) o el método 2 (usando el filtro `filtro2d_paso_bajo2` y eligiendo el cuantil 0.7).
...
A continuación, procedemos a comprimir y a reconstruir una imagen en blanco y negro, utilizando los dos filtros que actúan sobre la transformada wavelet discreta en 2D de la imagen. El segundo filtro, se probará para varios cuantiles (0.5, 0.7 y 0.9).
...
Ahora, se procede a generar una tabla que contiene información sobre el MSE, MAE y PSNR entre cada imagen en blanco y negro y la imagen reconstruida obtenida a partir de esta con el método 1 (usando el filtro `filtro2d_paso_bajo`) o el método 2 (usando el filtro `filtro2d_paso_bajo2` y eligiendo el cuantil 0.7).
