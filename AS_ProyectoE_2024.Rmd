---
title: "Compresión de imagen y de vídeo mediante transformada wavelet"
author:
  - Marcos Rosique
  - Pablo Selma
...
```{r 1_1_2_procesar_imagenes, echo=TRUE, include=FALSE}
...
```{r 1_1_3_ejemplo_array_a_imagen, echo=FALSE, include=FALSE ,warning=FALSE}
...
aplicar_dwt_filas <- function(matriz, filtro_wavelet = "haar") {
  lapply(1:nrow(matriz), function(i) wavelets::dwt(as.numeric(matriz[i, ]), 
                                                   filter = filtro_wavelet, n.levels=2))
}
...
extraccion_wavelet_filas <- function(array_imagen, filtro_wavelet = "haar") {
  ...
  return(list(wavelet_filas_R = wavelet_filas_R, 
              wavelet_filas_G = wavelet_filas_G, wavelet_filas_B = wavelet_filas_B))
}
...
aplicar_dwt_columnas <- function(matriz, filtro_wavelet = "haar") {
 lapply(1:ncol(matriz), function(j) wavelets::dwt(as.numeric(matriz[ , j]), 
                                                  filter = filtro_wavelet, n.levels=2))
}
...
extraccion_wavelet_columnas <- function(array_imagen, filtro_wavelet = "haar") {
  ...
  return(list(wavelet_columnas_R = wavelet_columnas_R,
              wavelet_columnas_G = wavelet_columnas_G, wavelet_columnas_B = wavelet_columnas_B))
}
```
...
```{r 2_1_2_ejemplo_1_2, echo=FALSE, out.width="50%"}
matriz_ceros <- matrix(0, nrow=dim(matriz_imagen)[1], ncol=dim(matriz_imagen)[2])
...
```{r 2_2_2_aplicar_a_concatenar, echo=FALSE, out.width="50%"}
...
```{r 2_2_2_aplicar_a_concatenar_correccion, echo=FALSE, include=FALSE, out.width="50%"}
matriz_imagen <- perro_01
...
filtro2d_paso_bajo2 <- function(dwt2d_imag, q) {
  dwt2d_imag$LH1 <- ifelse(abs(dwt2d_imag$LH1) <= quantile(abs(dwt2d_imag$LH1), q),
                           0, dwt2d_imag$LH1)
  dwt2d_imag$HL1 <- ifelse(abs(dwt2d_imag$HL1) <= quantile(abs(dwt2d_imag$HL1), q), 
                           0, dwt2d_imag$HL1)
  dwt2d_imag$HH1 <- ifelse(abs(dwt2d_imag$HH1) <= quantile(abs(dwt2d_imag$HH1), q), 
                           0, dwt2d_imag$HH1)
...
```{r 3_1_2_ejemplo_1, echo=FALSE, out.width="50%"}
grid.newpage()
...
```{r 3_1_2_ejemplo_2, echo=FALSE, out.width="50%"}
grid.newpage()
...
```{r 3_1_2_ejemplo_3, echo=FALSE, out.width="50%"}
grid.newpage()
...
```{r 3_1_2_ejemplo_4, echo=FALSE, out.width="50%"}
grid.newpage()
...
```{r 3_2_1_medidas_de_error_MSE, echo=TRUE, out.width="50%"}
mse_imags <- function(imag1, imag2, bn = FALSE) {
  if (bn) {
    return(mean((imag1 - imag2)^2))
  } else {
    return(mean(c(mean((imag1[,,1] - imag2[,,1])^2),
                  mean((imag1[,,2] - imag2[,,2])^2), mean((imag1[,,3] - imag2[,,3])^2))))
  }
}
```
...
mae_imags <- function(imag1, imag2, bn = FALSE) {
  if (bn) {
    ...
    return(mean(c(mean(abs(imag1[,,1] - imag2[,,1])),
                  mean(abs(imag1[,,2] - imag2[,,2])), mean(abs(imag1[,,3] - imag2[,,3])))))
  }
}
```
...
calc_errores <- function(imag1, imag2, bn = FALSE) {
  return(c("MSE" = mse_imags(imag1, imag2, bn), "MAE" = 
             mae_imags(imag1, imag2, bn), "PSNR" = psnr_imags(imag1, imag2, bn)))
}
```
...
```{r 3_3_5_ejemplo_bn_1, echo=FALSE, out.width="50%"}
grid.newpage()
grid.raster(imagen_rec1 / 255)
```
...
```{r 3_3_5_ejemplo_bn_2, echo=FALSE, out.width="50%"}
grid.newpage()
grid.raster(imagen_rec21 / 255)
```
...
```{r 3_3_5_ejemplo_bn_3, echo=FALSE, out.width="50%"}
grid.newpage()
grid.raster(imagen_rec22 / 255)
```
...
```{r 3_3_5_ejemplo_bn_4, echo=FALSE, out.width="50%"}
grid.newpage()
grid.raster(imagen_rec23 / 255)
```
...
```{r 5_1_3_1_visualizacion_fotogramas_extraidos, echo=TRUE, out.width="50%"}
...
La última función (`visualizar_imagen_RGB`) se encarga de dar una última comprobación de requisitos para la matriz RBG y representa la imagen una vez ya redimensionada para comprobar que todo ha sido correcto.

```{r 5_3_4_visualizar_matrices_RGB, echo=TRUE, include=FALSE}
...
# 5. Aplicación de Transformadas wavelet:

Aplicamos las transformaciones wavelet a cada vídeo para obtener:

```{r 5_1_1_transformada_wavelet_2d_por_fotograma, echo=FALSE}
...
aplic_dwt_concat_video <- function(video_frames, bn, filtro, nivel){
  dwt_frames <- c()
  for (i in 1:length(video_frames)){
    dwt_frames <- c(dwt_frames, 
                    list( aplicar_dwt_vector( imag = video_frames[[i]] ,bn=bn,filtro,nivel=nivel)) )
  }
  return(dwt_frames)
}
...
for (i in 1:length(normalized_matrices_list)){  #Iteramos sobre cada video
    dwt2d_videos<- c(dwt2d_videos, list( aplic_dwt2d_video( 
        normalized_matrices_list[[i]], bn=FALSE, wf = "haar", nivel=2) ) )
    dwt_concat_videos<- c(dwt_concat_videos, list( aplic_dwt_concat_video( 
        normalized_matrices_list[[i]], bn=FALSE, filtro = "haar", nivel=2) ) )
}
```
...
for (j in 1:length(normalized_matrices_list)){  #Iteramos sobre cada video
    ...
    for (i in 1:(length(videoframes_list[[j]]) -1 )){
      aux_trans <- c( aux_trans , list( normalized_matrices_list[[j]][[i+1]] -
                                          normalized_matrices_list[[j]][[i]] )  )
    }
    dwt_trans_2d_videos<- c(dwt_trans_2d_videos, list( 
        aplic_dwt2d_video( aux_trans, bn=FALSE, wf = "haar", nivel=2) ) )
    dwt_trans_concat_videos<- c(dwt_trans_concat_videos, list( 
        aplic_dwt_concat_video( aux_trans, bn=FALSE, filtro = "haar", nivel=2) ) )
}
```
...
* Compresión a través del segundo filtro de paso bajo del apartado 3.2 con un factor de 0.5.
...
Las funciones wavelet que filtran los coeficientes wavelet de video las basaremos en las respectivas que tienen la misma función para imagen.
...
Por último, creamos una función capaz de recuperar el vídeo a partir de los frames comprimidos, ya sea de manera usual, o transicional.

```{r 6_a_funcion_compresion_paso_bajo2_2, echo=FALSE}
...
rec_vid <- function(video_frames_wavelet_compressed, trans = FALSE){
  if (!trans) {
    ...
    for (i in 1:length(video_frames_wavelet_compressed)){
      ...
      video_frames <- c(video_frames, list(
        array(c(frame_r, frame_g, frame_b), dim = c(dim(frame_r), 3))))
    }
  } else {
    ...
    frame_inicial <- array(c(frame_inicial_r, frame_inicial_g,
                             frame_inicial_b), dim = c(dim(frame_inicial_r), 3))
    ...
    for (i in 2:length(video_frames_wavelet_compressed)){
      ...
      frame_b <- idwt.2d(video_frames_wavelet_compressed[[i]][[3]])
      video_frames <- c(video_frames, list(
        video_frames[[i-1]] + array(c(frame_r, frame_g, frame_b), dim = c(dim(frame_r), 3))))
    }
  }
  return(video_frames)
}
...
```{r concatenate_and_split_frames_functions, include=FALSE}
...
compress_video_concat_frames <- function( frames , filtroQ = FALSE, q = -1, nivel=2 ){ #Frames debe ser una lista con cada elemento un fotograma en formato Matriz cuadrada RGB 
  ...
  dwt2d_frames_concat   <- aplic_dwt(concatenated_frames,
                                     bn = bn, wf= "haar", nivel=nivel) #dwt.2d 
  
  concat_frames_comprimida_2d   <- filtrar_dwt(dwt2d_frames_concat
                                               , bn = bn, filtroQ = filtroQ, q = q)
  
  concat_frames_reconstruida2d <- reconstruir_imag(
      concat_frames_comprimida_2d, bn=FALSE, dim= dim(concatenated_frames))
  concat_frames_concat_filas_1d   <- comp_concatenada_filas_RGB(
      concatenated_frames, filtro = "haar", niveles=nivel)
  
  splitted_frames_comp2d <- split_frames(
      concat_frames_reconstruida2d, frame_height= N, frame_width= M)
  splitted_frames_concat_filas_1d <- split_frames(
      concat_frames_concat_filas_1d, frame_height= N, frame_width= M)
  
  return( list( splitted_frames_comp2d , splitted_frames_concat_filas_1d) )
}
```

```{r transponer_90_grados, include=FALSE, echo=FALSE}
...
```{r comprimir_video_frames_concatenados_prueba_1, include=FALSE}
comprimido_estatico <- compress_video_concat_frames(normalized_matrices_list[[1]] )
comprimido_giro <- compress_video_concat_frames(normalized_matrices_list[[2]] )

comprimido_estatico_concat_2d <- comprimido_estatico[[1]]  #Comprime la cuadrícula de fotogramas mediante dwt.2d
comprimido_estatico_concat_1d <- comprimido_estatico[[2]]  #Comprime la cuadrícula de fotogrames mediante la dwt 1d al vector que concatena los píxeles

comprimido_giro_concat_2d <- comprimido_giro[[1]]  #Comprime la cuadrícula de fotogramas mediante dwt.2d
comprimido_giro_concat_1d <- comprimido_giro[[2]]
...
error_compresion <- function(vid1, vid2) {
  ...
  cat("El error cuadrático en el último fotograma es",mse_imags(
      vid1[[length(vid1)]], vid2[[length(vid1)]], bn = FALSE),"\n")
}

error_compresion_silent <- function(vid1, vid2) {
  return( list(MSE_vid = mse_vid(vid1, vid2), MSE_last= mse_imags(
      vid1[[length(vid1)]], vid2[[length(vid1)]], bn = FALSE)))
}
```
...
comp_errores_estatico <- data.frame( rbind(
c("Independiente, sin compresión" , mse_vid(normalized_matrices_list[[1]],estatico_base), mse_imags(normalized_matrices_list[[1]][[length(normalized_matrices_list[[1]])]], estatico_base[[length(estatico_base)]], bn = FALSE)),
c("Transicional, sin compresión" , mse_vid(normalized_matrices_list[[1]],estatico_base_trans), mse_imags(normalized_matrices_list[[1]][[length(normalized_matrices_list[[1]])]], estatico_base_trans[[length(estatico_base_trans)]], bn = FALSE)),
c("Independiente, filtro", mse_vid(normalized_matrices_list[[1]],estatico_paso_bajo), mse_imags(normalized_matrices_list[[1]][[length(normalized_matrices_list[[1]])]], estatico_paso_bajo[[length(estatico_paso_bajo)]], bn = FALSE)),
c("Transicional, filtro", mse_vid(normalized_matrices_list[[1]],estatico_paso_bajo_trans), mse_imags(normalized_matrices_list[[1]][[length(normalized_matrices_list[[1]])]], estatico_paso_bajo_trans[[length(estatico_paso_bajo_trans)]], bn = FALSE)),
c("Independiente, medio filtro", mse_vid(normalized_matrices_list[[1]],estatico_paso_bajo2), mse_imags(normalized_matrices_list[[1]][[length(normalized_matrices_list[[1]])]], estatico_paso_bajo2[[length(estatico_paso_bajo2)]], bn = FALSE)),
c("Transicional, medio filtro", mse_vid(normalized_matrices_list[[1]],estatico_paso_bajo2_trans), mse_imags(normalized_matrices_list[[1]][[length(normalized_matrices_list[[1]])]], estatico_paso_bajo2_trans[[length(estatico_paso_bajo2_trans)]], bn = FALSE)),
c("Cuadrícula, dos dimensiones", mse_vid(normalized_matrices_list[[1]],comprimido_estatico_concat_2d),"---"),
c("Cuadrícula, una dimensión", mse_vid(normalized_matrices_list[[1]],comprimido_estatico_concat_1d),"---")   ) )
names(comp_errores_estatico)<- c("Método","Error total", "Error final")
print(comp_errores_estatico)
```
...
comp_errores_estatico <- data.frame( rbind(
c("Independiente, sin compresión" , mse_vid(normalized_matrices_list[[2]],giro_base), mse_imags(normalized_matrices_list[[2]][[length(normalized_matrices_list[[2]])]], giro_base[[length(giro_base)]], bn = FALSE)),
c("Transicional, sin compresión" , mse_vid(normalized_matrices_list[[2]], giro_base_trans), mse_imags(normalized_matrices_list[[2]][[length(normalized_matrices_list[[2]])]], giro_base_trans[[length(giro_base_trans)]], bn = FALSE)),
c("Independiente, paso bajo", mse_vid(normalized_matrices_list[[2]], giro_paso_bajo), mse_imags(normalized_matrices_list[[2]][[length(normalized_matrices_list[[2]])]], giro_paso_bajo[[length(giro_paso_bajo)]], bn = FALSE)),
c("Transicional, paso bajo", mse_vid(normalized_matrices_list[[2]],giro_paso_bajo_trans), mse_imags(normalized_matrices_list[[2]][[length(normalized_matrices_list[[2]])]], giro_paso_bajo_trans[[length(giro_paso_bajo_trans)]], bn = FALSE)),
c("Independiente, medio filtro", mse_vid(normalized_matrices_list[[2]],giro_paso_bajo2), mse_imags(normalized_matrices_list[[2]][[length(normalized_matrices_list[[2]])]], giro_paso_bajo2[[length(giro_paso_bajo2)]], bn = FALSE)),
c("Transicional, medio filtro", mse_vid(normalized_matrices_list[[2]],giro_paso_bajo2_trans), mse_imags(normalized_matrices_list[[2]][[length(normalized_matrices_list[[2]])]], giro_paso_bajo2_trans[[length(giro_paso_bajo2_trans)]], bn = FALSE)),
c("Cuadrícula, dos dimensiones", mse_vid(normalized_matrices_list[[2]],comprimido_giro_concat_2d),"---"),
c("Cuadrícula, una dimensión", mse_vid(normalized_matrices_list[[2]],comprimido_giro_concat_1d),"---")) )
names(comp_errores_estatico)<- c("Método","Error total", "Error final")
print(comp_errores_estatico)
```
...
prop_ceros <- data.frame( rbind(
  c("Fotograma a fotograma sin compresión", proporcion_ceros(dwt2d_videos[[1]]) * 100 ),
  c("Matrices transicionales sin compresión", proporcion_ceros(dwt_trans_2d_videos[[1]])*100),
  c("Fotograma a fotograma con filtro de paso bajo", proporcion_ceros(estatico_wavelet_comprimido_paso_bajo)*100),
  c("Matrices transicionales con filtro de paso bajo", proporcion_ceros(estatico_wavelet_comprimido_paso_bajo_trans)*100),
  c("Fotograma a fotograma con filtro de paso bajo 2 al 0.5", proporcion_ceros(estatico_wavelet_comprimido_paso_bajo2)*100),
  c("Matrices transicionales con filtro de paso bajo 2 al 0.5", proporcion_ceros(estatico_wavelet_comprimido_paso_bajo2_trans)*100)
))
names(prop_ceros)<- c("Método","Porcentaje de ceros")
print(prop_ceros)
```
...
prop_ceros_2 <- data.frame( rbind(
  c("Fotograma a fotograma sin compresión", proporcion_ceros(dwt2d_videos[[2]])*100 ),
  c("Matrices transicionales sin compresión", proporcion_ceros(dwt_trans_2d_videos[[2]])*100),
  c("Fotograma a fotograma con filtro de paso bajo", proporcion_ceros(giro_wavelet_comprimido_paso_bajo)*100),
  c("Matrices transicionales con filtro de paso bajo", proporcion_ceros(giro_wavelet_comprimido_paso_bajo_trans)*100),
  c("Fotograma a fotograma con filtro de paso bajo 2 al 0.5", proporcion_ceros(giro_wavelet_comprimido_paso_bajo2)*100),
  c("Matrices transicionales con filtro de paso bajo 2 al 0.5", proporcion_ceros(giro_wavelet_comprimido_paso_bajo2_trans)*100)
))
names(prop_ceros_2)<- c("Método","Porcentaje de ceros")
print(prop_ceros_2)
```
