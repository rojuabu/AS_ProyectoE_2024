---
title: "Compresión de imagen y video mediante transformada wavelet \n\n  Marcos Rosique, Pablo Selma, Víctor Mateu Izquierdo,Marc Velasco Mateu, Carlos Ribes García, Rodrigo Juanes"
date: "2024-12-24"
output:
...
```{r 5_2_1_fun_frames_a_matrices, echo=TRUE}
...
```{r 6_a_funcion_compresion_paso_bajo2_2, echo=TRUE}
...
Vamos a construir el algoritmo de compresión propiamente dicho (las funciones concatenate_frames y split_frames son una la inversa de la otra y aglomeran o separan los fotogramas según se ha explicado anteriormente):

```{r comprimir_video_frames_concatenados}
compress_video_concat_frames <- function( frames , filtroQ = FALSE, q = -1, nivel=2 ){ 
#Frames debe ser una lista con cada elemento un fotograma en formato Matriz cuadrada RGB 
...
c("Fotograma a fotograma sin compresión" , error_compresion_silent(normalized_matrices_list[[2]],estatico_base)),
c("Matrices transicionales sin compresión" , error_compresion_silent(normalized_matrices_list[[2]],estatico_base_trans)),
c("Fotograma a fotograma con filtro de paso bajo",error_compresion_silent(normalized_matrices_list[[2]],estatico_paso_bajo)),
c("Matrices transicionales con filtro de paso bajo",error_compresion_silent(normalized_matrices_list[[2]],estatico_paso_bajo_trans)),
c("Fotograma a fotograma con filtro de paso bajo 2 al 0.5.",error_compresion_silent(normalized_matrices_list[[2]],estatico_paso_bajo2)),
c("Fotograma a fotograma con filtro de paso bajo 2 al 0.5", error_compresion_silent(normalized_matrices_list[[2]],estatico_paso_bajo2_trans)),
c("Cuadrícula de fotogramas dwt.2d" , error_compresion_silent(normalized_matrices_list[[2]],comprimido_concat_2d)),
c("Cuadrícula de fotogramas dwt 1d" , error_compresion_silent(normalized_matrices_list[[2]],comprimido_concat_1d))   ) )
names(comp_errores_estatico)<- c("Método","Error")
print(comp_errores_estatico[1:2])
```
...
comp_errores_giro <- data.frame( rbind(
c("Fotograma a fotograma sin compresión" , error_compresion_silent(normalized_matrices_list[[2]],giro_base)),
c("Matrices transicionales sin compresión" , error_compresion_silent(normalized_matrices_list[[2]],giro_base_trans)),
c("Fotograma a fotograma con filtro de paso bajo",error_compresion_silent(normalized_matrices_list[[2]],giro_paso_bajo)),
c("Matrices transicionales con filtro de paso bajo",error_compresion_silent(normalized_matrices_list[[2]],giro_paso_bajo_trans)),
c("Fotograma a fotograma con filtro de paso bajo 2 al 0.5.",error_compresion_silent(normalized_matrices_list[[2]],giro_paso_bajo2)),
c("Fotograma a fotograma con filtro de paso bajo 2 al 0.5", error_compresion_silent(normalized_matrices_list[[2]],giro_paso_bajo2_trans)),
c("Cuadrícula de fotogramas dwt.2d" , error_compresion_silent(normalized_matrices_list[[2]],comprimido_concat_2d)),
c("Cuadrícula de fotogramas dwt 1d" , error_compresion_silent(normalized_matrices_list[[2]],comprimido_concat_1d))) )
names(comp_errores_giro)<- c("Método","Error")
print(comp_errores_giro[1:2])
```
...
La siguiente función tiene este propósito.
...
names(prop_ceros)<- c("Método","Ratio")
print(prop_ceros)
```
...
```{r 6_c_comparacion_compresion_2, echo=FALSE, include=FALSE}
cat("","Método: Fotograma a fotograma sin compresión.","\n")
...
names(prop_ceros_2)<- c("Método","Ratio")
print(prop_ceros_2)
```
...
Por último, el filtro de paso bajo aplica una compresión de un 37%, aunque este factor es completamente dependiente de nuestro umbral de compresión aplicado. Podría ser tan bajo como 0%, o tan alto como 75%.
