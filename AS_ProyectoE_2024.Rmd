...
matriz_concatenada <- dwt_concatenada_filas(perro_01)
...
```
La variable dwt_videos es una lista que contiene:

  - Una lista por cada vídeo que contiene     (dwt_videos[[video_i]]):
  - Una lista por cada fotograma del vídeo que contiene (dwt_videos[[video_i]][[fotograma_j]]):
  - La transformada wavelet del canal R [[1]], G [[2]] o B [[3]]


```{r 5_1_1_transformada_wavelet_concatenado }

aplic_dwt_concat_video <- function(video_frames, bn, wf, nivel){
  dwt_frames <- c()
  for (i in 1:length(video_frames)){
    dwt_frames <- c(dwt_frames, list( aplic_dwt( imag = video_frames[[i]] ,bn=bn,wf=wf,nivel=nivel)) )
  }
...
for (names in video_files){  #Iteramos sobre cada video
    ...
    aux_rgb <- convert_frames_to_rgb(aux_frames)
    aux_norm <- normalize_matrices(aux_rgb)
    dwt2d_videos<- c(dwt2d_videos, list( aplic_dwt_video( aux_norm, bn=FALSE, wf = "haar", nivel=2) ) )
}
```
