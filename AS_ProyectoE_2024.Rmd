...
dwt_concatenada_filas <- function(matriz_imagen_color, filtro="haar", niveles=2){
...
for (names in video_files){  #Iteramos sobre cada video
    ...
    dwt2d_videos<- c(dwt2d_videos, list( aplic_dwt2d_video( aux_norm, bn=FALSE, wf = "haar", nivel=2) ) )
    dwt_concat_videos<- c(dwt_concat_videos, list( aplic_dwt_concat_video( aux_norm, bn=FALSE, filtro = "haar", nivel=2) ) )
}
```
...
for (names in video_files){  #Iteramos sobre cada video
    ...
    aux_rgb <- convert_frames_to_rgb(aux_frames)
    aux_norm <- normalize_matrices(aux_rgb)
    aux_trans <-  aux_norm[1] 
    for (i in 1:(length(aux_frames) -1 )){
      aux_trans <- c( aux_trans , list( aux_norm[[i+1]] - aux_norm[[i]] )  )
    }
...
```{r concatenate_and_split_pruebas_1}
aux_frames <- extract_frames_in_memory(video_files[2], fps = 1)
aux_rgb <- convert_frames_to_rgb(aux_frames)
aux_norm <- normalize_matrices(aux_rgb)
concatenated_frames <- concatenate_frames(aux_norm)
visualizar_imagen_RGB(concatenated_frames)

```

```{r concatenate_and_split_pruebas_2}
rows <- nrow(aux_norm[[1]])
cols <- ncol(aux_norm[[1]])
lista <- split_frames(concatenated_frames, rows , cols )
```

Ahora construimos un algoritmo de compresión que consista en comprimir la imagen resultante de concatenar todos los fotogramas:

```{r comprimir_video_frames_concatenados}

compress_video_concat_frames <- function( frames , filtroQ = FALSE ,nivel=2 ){ #Frames debe ser una lista con cada elemento un fotograma en formato Matriz cuadrada RGB 
  ...
  dwt_frames_concat   <- aplic_dwt(concatenated_frames, bn = bn, wf= "haar", nivel=nivel)
  concat_frames_comprimida   <- comprimir_imag(dwt_frames_concat, bn = bn, filtroQ= filtroQ, q = -1)
  concat_frames_reconstruida <- reconstruir_imag(concat_frames_comprimida, bn=FALSE, dim= dim(concatenated_frames))
  
  splitted_frames_comp <- split_frames(concat_frames_reconstruida, frame_height= N, frame_width= M)
  
  return( splitted_frames_comp)
}
```
...
aux_rgb <- convert_frames_to_rgb(aux_frames)
aux_norm <- normalize_matrices(aux_rgb)
comprimido<-compress_video_concat_frames(aux_norm)
visualizar_imagen_RGB(comprimido[[1]])
comprimido_rotado<- rotate_frames_90( rotate_frames_90( rotate_frames_90( comprimido) ) )
visualizar_imagen_RGB(comprimido_rotado[[1]])
```
