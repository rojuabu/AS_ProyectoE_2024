...
normalize_matrices_no_funciona <- function(rgb_matrices, target_width = 128, target_height = 128) {
  normalized_matrices <- lapply(rgb_matrices, function(matrix) {
    ...
    normalized_matrix <- as.integer(image_data(resized_img)) # Convertir de nuevo a matriz
    normalized_matrix
  })
  normalized_matrices
}
...
```{r 4_3_1_fun_normalizar_matrices_v2, echo=FALSE}
...
```
```{r 4_3_3_visualizar_matrices_RGB_no_funciona, echo=FALSE, include=FALSE}

visualizar_imagen_RGB_no_funciona <- function(matriz_RGB) {
  ...
  # Convertir la matriz RGB a un objeto raster
  img <- as.raster(matriz_RGB)
...
Nuestro primer algoritmo de reconstrucción aplicará una transformada inversa de wavelet para recuperar el primer fotograma original, y después aplicara la misma transformación inversa a las diferencias entre fotogramas.

```{r 6_a_reconstruccion_directa, echo=FALSE}
idwt.2d(dwt2d_videos[[1]][[1]][[1]])
visualizar_imagen_RGB(idwt.2d(dwt2d_videos[[1]][[1]][[1]]))
```

### b. Medición del error entre el video original y el comprimido.

```{r 6_b_medición_error, echo=FALSE}
