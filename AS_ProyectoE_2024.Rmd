...
for (names in video_files){  #Iteramos sobre cada video
    ...
    videos_matrix_norm <- c( videos_matrix_norm , aux_norm)
}
```
...
for (i in 1:length(videos_matrix_norm)){  #Iteramos sobre cada video
    ...
    aux_trans <-  videos_matrix_norm[[1]][1] 
    for (i in 1:(length(aux_frames) -1 )){
      aux_trans <- c( aux_trans , list( videos_matrix_norm[[1]][[i+1]] - videos_matrix_norm[[1]][[i]] )  )
    }
...
compress_video_concat_frames <- function( frames , filtroQ = FALSE, q = -1, nivel=2 ){ #Frames debe ser una lista con cada elemento un fotograma en formato Matriz cuadrada RGB 
  ...
  concat_frames_concat_filas_1d   <- comp_concatenada_filas_RGB(concatenated_frames, filtro = "haar", niveles=niveles)
  
  splitted_frames_comp2d <- split_frames(concat_frames_reconstruida2d, frame_height= N, frame_width= M)
  splitted_frames_concat_filas_1d <- split_frames(cconcat_frames_concat_filas_1d, frame_height= N, frame_width= M)
  
  return( list( splitted_frames_comp , splitted_frames_concat_filas_1d) )
}
```
...
visualizar_imagen_RGB(comprimido[[1]][[1]])
